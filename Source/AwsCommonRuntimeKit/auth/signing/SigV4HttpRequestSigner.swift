...
class SigV4HttpRequestSigner {
    ...
    public func signRequest(request: HttpRequest, config: SigningConfig, callback: @escaping OnRequestSigningComplete) throws -> Bool {
        ...
        let configMutablePointer: UnsafeMutablePointer<aws_signing_config_aws>?
        configMutablePointer = UnsafeMutablePointer<aws_signing_config_aws>.allocate(capacity: 1)
        configMutablePointer?.initialize(to: config.rawValue)
        ...
        return aws_sign_request_aws(allocator.rawValue,
                                    ...
                                    if errorCode == AWS_OP_SUCCESS {
                                        aws_apply_signing_result_to_http_request(request.rawValue, allocator.rawValue, signingResult)
                                    }
                                    callback.pointee.onRequestSigningComplete(request, errorCode)
                                    },
