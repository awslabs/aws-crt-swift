...
public struct SigningConfig: CStructWithUserData, @unchecked Sendable {
  ...
  public init(
    algorithm: SigningAlgorithmType,
  ...
  func withCStruct<Result>(
    userData: UnsafeMutableRawPointer?, _ body: (aws_signing_config_aws) -> Result
  ) -> Result {
    ...
    return withByteCursorFromStrings(
      region,
      service,
      signedBodyValue.description
    ) { regionCursor, serviceCursor, signedBodyValueCursor in
...
private func onShouldSignHeader(
  nameCursor: UnsafePointer<aws_byte_cursor>!,
