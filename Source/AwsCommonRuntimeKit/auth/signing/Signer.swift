...
public class Signer {
  ...
  public static func signRequest(
    ...
    return try await withCheckedThrowingContinuation {
      ...
      let signRequestCore = SignRequestCore(
        request: request,
  ...
  public static func signChunk(
    ...
    guard
      let signable = previousSignature.withByteCursorPointer({ previousSignatureCursor in
        aws_signable_new_chunk(
          allocator.rawValue, iStreamCore.rawValue, previousSignatureCursor.pointee)
      })
    else {
      throw CommonRunTimeError.crtError(.makeFromLastError())
    }
  ...
  public static func signEvent(
    ...
    guard config.signatureType == SignatureType.requestEvent else {
      throw CommonRunTimeError.crtError(
        .init(code: AWS_AUTH_SIGNING_MISMATCHED_CONFIGURATION.rawValue))
    }
...
private func onRequestSigningComplete(
  signingResult: UnsafeMutablePointer<aws_signing_result>?,
  errorCode: Int32,
  ...
  // Success
  let signedRequest = aws_apply_signing_result_to_http_request(
    signRequestCore.request.rawValue,
    allocator.rawValue,
    signingResult!)
...
private func onSigningComplete(
  signingResult: UnsafeMutablePointer<aws_signing_result>?,
  errorCode: Int32,
  userData: UnsafeMutableRawPointer!
) {
  ...
  guard errorCode == AWS_OP_SUCCESS else {
    chunkSignerCore.continuation.resume(
      throwing: CommonRunTimeError.crtError(CRTError(code: errorCode)))
    return
  }
  ...
  guard
    aws_signing_result_get_property(
      signingResult!,
      signature.rawValue,
      &awsStringPointer) == AWS_OP_SUCCESS
  else {
