...
public class IMDSClient {
    ...
    let imdsCallbackCore: IMDSResourceCallbackCore
    public init(bootstrap: ClientBootstrap,
        ...
        bootstrap = bootstrap.rawValue
        retry_strategy = retryStrategy.rawValue
        imds_version = protocolVersion.rawValue
    ...
    public func getAmiId() async throws -> String? {
        return try await withCheckedThrowingContinuation({ (continuation: ResourceContinuation) in
            let callbackData = IMDSResourceCallbackCore(continuation: continuation)
...
private func resourceCallback(_ byteBuf: UnsafePointer<aws_byte_buf>?,
    ...
    let pointer = userData.assumingMemoryBound(to: IMDSResourceCallbackCore.self)
