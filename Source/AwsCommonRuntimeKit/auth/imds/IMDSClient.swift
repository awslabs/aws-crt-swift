...
public class IMDSClient: @unchecked Sendable {
  ...
  func getResourceWithPath(
    ...
    resourcePath.withByteCursor { resourcePathCursor in
      if (aws_imds_client_get_resource_async(
        client.rawValue,
        resourcePathCursor,
        onGetResource,
...
private func onGetResourceList(
  arrayListPointer: UnsafePointer<aws_array_list>?,
  errorCode: Int32,
  userData: UnsafeMutableRawPointer!
) {
  let imdsClientCore = Unmanaged<ContinuationCore<[String]>>.fromOpaque(userData)
    .takeRetainedValue()
  if errorCode != AWS_OP_SUCCESS {
    imdsClientCore.continuation.resume(
      throwing: CommonRunTimeError.crtError(CRTError(code: errorCode)))
  ...
  // Success
  imdsClientCore.continuation.resume(
    returning: arrayListPointer!.pointee.byteCursorListToStringArray())
}

private func onGetCredentials(
  credentialsPointer: OpaquePointer?,
  errorCode: Int32,
  userData: UnsafeMutableRawPointer!
) {
  let imdsClientCore = Unmanaged<ContinuationCore<Credentials>>.fromOpaque(userData)
    .takeRetainedValue()
  if errorCode != AWS_OP_SUCCESS {
    imdsClientCore.continuation.resume(
      throwing: CommonRunTimeError.crtError(CRTError(code: errorCode)))
...
private func onGetIAMProfile(
  profilePointer: UnsafePointer<aws_imds_iam_profile>?,
  errorCode: Int32,
  userData: UnsafeMutableRawPointer!
) {
  let imdsClientCore = Unmanaged<ContinuationCore<IAMProfile>>.fromOpaque(userData)
    .takeRetainedValue()
  if errorCode != AWS_OP_SUCCESS {
    imdsClientCore.continuation.resume(
      throwing: CommonRunTimeError.crtError(CRTError(code: errorCode)))
...
private func onGetInstanceInfo(
  infoPointer: UnsafePointer<aws_imds_instance_info>?,
  errorCode: Int32,
  userData: UnsafeMutableRawPointer!
) {
  let imdsClientCore = Unmanaged<ContinuationCore<IMDSInstanceInfo>>.fromOpaque(userData)
    .takeRetainedValue()
  if errorCode != AWS_OP_SUCCESS {
    imdsClientCore.continuation.resume(
      throwing: CommonRunTimeError.crtError(CRTError(code: errorCode)))
  ...
  // Success
  imdsClientCore.continuation.resume(
    returning: IMDSInstanceInfo(instanceInfo: infoPointer!.pointee))
}
