...
open class AWSCredentialsProvider: CredentialsProvider {
    ...
    required public init(connection: UnsafeMutablePointer<aws_credentials_provider>, allocator: Allocator = defaultAllocator) {
    ...
    func createWrappedProvider(cProvider: UnsafeMutablePointer<aws_credentials_provider>, allocator: Allocator) -> CredentialsProvider {
    ...
    func createCredentialsProviderStatic(config: CredentialsProviderStaticConfigOptions) -> CredentialsProvider? {
        ...
        guard let provider = aws_credentials_provider_new_static(allocator.rawValue, config.rawValue) else { return nil }

        return createWrappedProvider(cProvider: provider, allocator: allocator)
    }
    
    func createCredentialsProviderEnvironment(shutdownOptions: AWSCredentialsProviderShutdownOptions) -> CredentialsProvider? {
        let options = UnsafeMutablePointer<aws_credentials_provider_environment_options>.allocate(capacity: 1)
        options.pointee = aws_credentials_provider_environment_options(shutdown_options: shutdownOptions.rawValue.pointee)
        guard let provider = aws_credentials_provider_new_environment(allocator.rawValue, options) else {return nil}
        defer {
            options.deallocate()
        }
        return createWrappedProvider(cProvider: provider, allocator: allocator)
    ...
    func createCredentialsProviderProfile(profileOptions: AWSCredentialsProviderProfileOptions) -> CredentialsProvider? {
        ...
        options.pointee = aws_credentials_provider_profile_options(shutdown_options: profileOptions.shutdownOptions.rawValue.pointee,
                                                                   ...
                                                                   bootstrap: nil,
                                                                   function_table: nil)
        guard let provider = aws_credentials_provider_new_profile(allocator.rawValue, options) else {return nil}
        defer {
            options.deallocate()
        }
        return createWrappedProvider(cProvider: provider, allocator: allocator)
    ...
    func createCredentialsProviderImds(imdsConfig: AWSCredentialsProviderImdsConfig) -> CredentialsProvider? {
        ...
        options.pointee = aws_credentials_provider_imds_options(shutdown_options: imdsConfig.shutdownOptions.rawValue.pointee, bootstrap: imdsConfig.bootstrap.rawValue, imds_version: IMDS_PROTOCOL_V2, function_table: nil)
    ...
    func createCredentialsProviderChain() -> CredentialsProvider? {
        ...
        guard let provider = aws_credentials_provider_new_chain(allocator.rawValue, nil) else {return nil}
        return createWrappedProvider(cProvider: provider, allocator: allocator)
    }
