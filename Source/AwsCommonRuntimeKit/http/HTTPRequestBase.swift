...
extension HTTPRequestBase {
  ...
  public func addHeaders(headers: [HTTPHeader]) {
  ...
  public func getHeaderValue(name: String) -> String? {
    let headers = aws_http_message_get_headers(rawValue)
    var value = aws_byte_cursor()

    guard
      (name.withByteCursor { nameCursor in
  ...
  public func getHeaders() -> [HTTPHeader] {
