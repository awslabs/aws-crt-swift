...
private func onChangeSettingsComplete(
...
private func onPingComplete(
  connection: UnsafeMutablePointer<aws_http_connection>?,
  roundTripTimeNs: UInt64,
  errorCode: Int32,
  userData: UnsafeMutableRawPointer!
) {
  let continuation = Unmanaged<ContinuationCore<UInt64>>.fromOpaque(userData).takeRetainedValue()
    .continuation
