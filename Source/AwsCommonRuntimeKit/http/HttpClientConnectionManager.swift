...
public class HttpClientConnectionManager {
    ...
    public init(options: HttpClientConnectionOptions, allocator: Allocator = defaultAllocator) {
        ...
        let shutDownPtr: UnsafeMutablePointer<ShutDownCallbackOptions>? = fromOptionalPointer(
            ptr: options.shutDownOptions
        )
        // Todo: add new http2 options as properties in HttpClientConnectionOptions
        var mgrOptions = aws_http_connection_manager_options(
