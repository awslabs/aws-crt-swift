...
public class HttpClientConnectionOptions {
    ...
    func withCManagerOptions<Result>(shutdownOptions: aws_shutdown_callback_options,
                                     _ body: (UnsafePointer<aws_http_connection_manager_options>) -> Result
    ) -> Result {
        return hostName.withByteCursor { hostNameCursor in
            return HttpProxyOptions.withCPointer(proxyOptions: proxyOptions) { proxyPointer in
                ...
                return withUnsafePointer(to: cManagerOptions, { body($0) })
            }
        }
