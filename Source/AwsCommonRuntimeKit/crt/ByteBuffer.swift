...
extension ByteBuffer: AwsStream {
    public var status: aws_stream_status {
        aws_stream_status(is_end_of_stream: self.currentIndex == array.count, is_valid: true)
    }
...
public extension ByteBuffer {
    ...
    /// - Returns: `FileHandle`
    func toFile(filePath: String) -> FileHandle? {
    ...
    ///
    /// - Returns: `InputStream`
    func toStream() -> InputStream {
        InputStream(data: self.toData())
    }

    internal func toAwsByteBuf(allocator: Allocator = defaultAllocator) -> aws_byte_buf {
        let count = self.array.count
        return self.array.withUnsafeMutableBufferPointer { pointer in
            aws_byte_buf(len: count,
...
public extension ByteBuffer {
    ...
    func base64EncodedSha256(allocator: Allocator = defaultAllocator, truncate: Int = 0) -> String {
        sha256(allocator: allocator, truncate: truncate).toData().base64EncodedString()
    }
