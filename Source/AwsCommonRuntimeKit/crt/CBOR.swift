...
public enum CBORType: Equatable {
  ...
  ///
  /// It is up to the caller to add/pop the correct number of key-value pairs to the map.
...
public class CBORDecoder {
  ...
  private func decodeUInt() throws -> CBORType {
    ...
    guard aws_cbor_decoder_pop_next_unsigned_int_val(self.rawValue, &out_value) == AWS_OP_SUCCESS
    else {
      throw CommonRunTimeError.crtError(.makeFromLastError())
    }
  ...
  private func decodeNegInt() throws -> CBORType {
    var out_value: UInt64 = 0
    guard aws_cbor_decoder_pop_next_negative_int_val(self.rawValue, &out_value) == AWS_OP_SUCCESS
    else {
      throw CommonRunTimeError.crtError(.makeFromLastError())
    }
