...
class ShutdownCallbackCore {
  ...
  init(
    _ shutdownCallback: ShutdownCallback?,
    data: AnyObject? = nil
  ) {
  ...
  func getRetainedShutdownOptions() -> aws_shutdown_callback_options {
    ...
    shutdown_options.shutdown_callback_fn = { userData in
      let shutdownCallbackOptions = Unmanaged<ShutdownCallbackCore>.fromOpaque(userData!)
        .takeRetainedValue()
      shutdownCallbackOptions.shutdownCallback()
    }
    shutdown_options.shutdown_callback_user_data = Unmanaged<ShutdownCallbackCore>.passRetained(
      self
    ).toOpaque()
  ...
  func getRetainedCredentialProviderShutdownOptions() -> aws_credentials_provider_shutdown_options {
    ...
    shutdown_options.shutdown_user_data = Unmanaged<ShutdownCallbackCore>.passRetained(self)
      .toOpaque()
    return shutdown_options
  }
