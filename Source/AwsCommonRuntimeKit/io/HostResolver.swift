...
public class HostResolver: HostResolverProtocol, @unchecked Sendable {
  ...
  public init(
    eventLoopGroup: EventLoopGroup, maxHosts: Int, maxTTL: Int,
    shutdownCallback: ShutdownCallback? = nil
  ) throws {
    let shutdownCallbackCore = ShutdownCallbackCore(shutdownCallback)
    let getRawValue: () -> UnsafeMutablePointer<aws_host_resolver>? = {
      withUnsafePointer(to: shutdownCallbackCore.getRetainedShutdownOptions()) {
        shutdownCallbackCorePointer in
  ...
  public func resolveAddress(args: HostResolverArguments) async throws -> [HostAddress] {
    ...
    return try await withCheckedThrowingContinuation({
      (continuation: CheckedContinuation<[HostAddress], Error>) in
  ...
  public func purgeCache(args: HostResolverArguments) async {
    ...
    return await withCheckedContinuation({ (continuation: CheckedContinuation<(), Never>) in
      ...
      guard aws_host_resolver_purge_host_cache(rawValue, &purgeCacheOptions) == AWS_OP_SUCCESS
      else {
        continuationCore.release()
        continuation.resume()
        return
...
private func onHostResolved(
  _ resolver: UnsafeMutablePointer<aws_host_resolver>?,
  _ hostName: UnsafePointer<aws_string>?,
  _ errorCode: Int32,
  _ hostAddresses: UnsafePointer<aws_array_list>?,
  _ userData: UnsafeMutableRawPointer!
) {
  let hostResolverCore = Unmanaged<ContinuationCore<[HostAddress]>>.fromOpaque(userData)
    .takeRetainedValue()
  guard errorCode == AWS_OP_SUCCESS else {
    hostResolverCore.continuation.resume(
