...
public class FileBasedConfiguration: @unchecked Sendable {
  ...
  public init(
    ...
    guard
      let credentialsFilePath = withOptionalByteCursorPointerFromString(
        credentialsFilePath,
        {
    ...
    guard
      let configFilePath = withOptionalByteCursorPointerFromString(
        configFilePath,
        {
    ...
    guard
      let rawValue = aws_profile_collection_new_from_merge(
        allocator.rawValue,
        configCollection,
  ...
  public static func resolveConfigPath(
    ...
    guard
      let filePath: UnsafeMutablePointer<aws_string> = withOptionalByteCursorPointerFromString(
        overridePath,
        { path in
          switch sourceType {
  ...
  public func getSection(
    ...
    guard
      let sectionPointer = aws_profile_collection_get_section(
        self.rawValue,
...
extension FileBasedConfiguration.Section {
  ...
  public class Property: @unchecked Sendable {
    ...
    public func getSubProperty(name: String) -> String? {
      let awsString = AWSString(name)
      guard let stringPointer = aws_profile_property_get_sub_property(rawValue, awsString.rawValue)
      else {
        return nil
      }
