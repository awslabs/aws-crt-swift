...
public class EventStreamMessageDecoder {
  ...
  public init(
    onPayloadSegment: @escaping OnPayloadSegment,
    ...
    aws_event_stream_streaming_decoder_init_from_options(
      &rawValue, allocator.rawValue, &decoderOptions)
  }
...
private func onHeaderReceivedFn(
  ...
  let name = withUnsafeBytes(
    of: header.pointee.header_name
  ) { (namePointer) -> String in
