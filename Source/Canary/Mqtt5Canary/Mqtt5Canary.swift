...
let NANO_SECOND: UInt64 = 1_000_000_000
let SLEEP_OVERHEAD: UInt64 = 2_000_000 // 2ms overhead
let UNSUBSRIBE_TESTING_TOPIC = "testTopic1"
...
class Mqtt5CanaryTestContext {
    ...
    /// Setup Test Context
    func appendClient(client: Mqtt5CanaryClient){
        mqtt5Clients.append(client)
    }
    
    func setClientConnection(index: Int, connected: Bool){
        mqtt5Clients[index].setConnected(connected: connected)
    }
    ...
    func mqtt5CanaryRunOperation(_ operation: Mqtt5CanaryOperation, _ index: Int) async throws{
        ...
        case Mqtt5CanaryOperation.UNSUBSCRIBE:
            if (mqtt5Clients[index].subscriptionCount > 0) {
                try await mqtt5CanaryOperationUnsubscribe(clientIndex: index, testTopic: mqtt5Clients[index].getUnsubscribedTopic())
            } else {
        ...
        case Mqtt5CanaryOperation.PUBLISH_TO_SUBSCRIBED_TOPIC_QOS0:
            try await mqtt5CanaryOperationPublish(clientIndex: index, topic: mqtt5Clients[index].getSubscribedTopic(), qos: QoS.atMostOnce)
        case Mqtt5CanaryOperation.PUBLISH_TO_SUBSCRIBED_TOPIC_QOS1:
            try await mqtt5CanaryOperationPublish(clientIndex: index, topic: mqtt5Clients[index].getSubscribedTopic(), qos: QoS.atLeastOnce)
        case Mqtt5CanaryOperation.PUBLISH_TO_SHARED_TOPIC_QOS0:
            try await mqtt5CanaryOperationPublish(clientIndex: index, topic: mqtt5Clients[index].shared_topic, qos: QoS.atMostOnce)
        case Mqtt5CanaryOperation.PUBLISH_TO_SHARED_TOPIC_QOS1:
            try await mqtt5CanaryOperationPublish(clientIndex: index, topic: mqtt5Clients[index].shared_topic, qos: QoS.atLeastOnce)
            
        case Mqtt5CanaryOperation.NULL:
            fallthrough
        case Mqtt5CanaryOperation.DESTROY:
            fallthrough
    ...
    func mqtt5CanaryOperationStart(clientIndex: Int) async throws{
        if !mqtt5Clients[clientIndex].is_connected {
            await statistic.incrementTotalOperation()
            try mqtt5Clients[clientIndex].client.start()
        }
    }
    ...
    func mqtt5CanaryOperationStop(clientIndex: Int) async throws {
        if mqtt5Clients[clientIndex].is_connected {
            await statistic.incrementTotalOperation()
            try mqtt5Clients[clientIndex].client.stop()
            // clean up the subscription count
            mqtt5Clients[clientIndex].subscriptionCount = 0
        }
    }
    ...
    func mqtt5CanaryOperationSubscribe(clientIndex: Int) async throws{
        ...
        do {
            let canaryClient = mqtt5Clients[clientIndex]
            ...
            if(canaryClient.subscriptionCount == 1){
                subscriptions.append(Subscription(topicFilter: canaryClient.shared_topic, qos: QoS.atLeastOnce))
            }
            
            let suback = try await canaryClient.client.subscribe(subscribePacket: SubscribePacket(subscriptions: subscriptions))
    ...
    func mqtt5CanaryOperationUnsubscribe(clientIndex: Int, testTopic : String = UNSUBSRIBE_TESTING_TOPIC) async throws{
        if !mqtt5Clients[clientIndex].is_connected {
                return try await mqtt5CanaryOperationStart(clientIndex: clientIndex)
            }
        ...
        do {
            ...
            if unsubAck.reasonCodes[0] == UnsubackReasonCode.success{
                ...
                print("Unsubscribe to topic \(testTopic), failed with reason \(unsubAck.reasonCodes[0])")
            }
        }catch{
            await self.statistic.incrementUnsubscribeFailed()
            print("Unsubscribe to topic \(testTopic), operation failed")
        }
    }
    
    func mqtt5CanaryOperationPublish(clientIndex: Int, topic: String = UNSUBSRIBE_TESTING_TOPIC, qos: QoS) async throws{
        if !mqtt5Clients[clientIndex].is_connected {
                return try await mqtt5CanaryOperationStart(clientIndex: clientIndex)
            }
        ...
        do {
            ...
            if qos == QoS.atMostOnce ||
                puback.puback?.reasonCode == PubackReasonCode.success ||
...
actor Mqtt5CanaryStatistic {
    ...
    func printStatistics(duration: TimeInterval) {
        let subscribeSuccessRate = subscribeAttempt > 0 ? Double(subscribeSucceed) / Double(subscribeAttempt) * 100 : 0
        let publishSuccessRate = publishAttempt > 0 ? Double(publishSucceed) / Double(publishAttempt) * 100 : 0
        let unsubscribeSuccessRate = unsubAttempt > 0 ? Double(unsubSucceed) / Double(unsubAttempt) * 100 : 0
        ...
        print("Subscribe - Attempts: \(subscribeAttempt), Success: \(subscribeSucceed), Failed: \(subscribeFailed), Success Rate: \(String(format: "%.2f", subscribeSuccessRate))%")
        print("Publish - Attempts: \(publishAttempt), Success: \(publishSucceed), Failed: \(publishFailed), Success Rate: \(String(format: "%.2f", publishSuccessRate))%")
        print("Unsubscribe - Attempts: \(unsubAttempt), Success: \(unsubSucceed), Failed: \(unsubFailed), Success Rate: \(String(format: "%.2f", unsubscribeSuccessRate))%")
    }
...
struct Mqtt5CanaryTestOptions {
    ...
    init(testApp: Mqtt5Canary) throws{
        ...
        self.tpsSleepTime = testApp.tps > 0 ? (UInt64(NANO_SECOND / testApp.tps)) : 0
        print("tps sleep time: \(self.tpsSleepTime)")
...
func Mqtt5CanaryTestRunIteration(_ context: Mqtt5CanaryTestContext, _ options: Mqtt5CanaryTestOptions) async throws -> Void {
    let operationIndex = Int.random(in : 0..<options.operationDistribution.count)
    let clientIndex = Int.random(in: 0..<context.mqtt5Clients.count)
    try await context.mqtt5CanaryRunOperation(options.operationDistribution[operationIndex], clientIndex)
}
...
struct Mqtt5Canary: AsyncParsableCommand {
    ...
    @Option(name: .long, help: "Path to a PEM encoded certificate to use with mTLS. Required if useTls is True.")
    var cert: String? = nil
    
    @Option(name: .long, help: "Path to a PEM encoded private key that matches cert. Required if useTls is True.")
    var key: String? = nil
    ...
    mutating func createClient(context: Mqtt5CanaryTestContext, index: Int, testOptions: Mqtt5CanaryTestOptions) async throws{
        let clientId = "Mqtt5CanaryTest-" + UUID().uuidString
        let connectionOption = MqttConnectOptions(clientId: clientId)
        let clientOption = MqttClientOptions(hostName: self.endpoint, port: self.port, bootstrap: testOptions.boostrap, tlsCtx: testOptions.tlsctx,
        ...
        let canaryClient = Mqtt5CanaryClient(client: client, clientId: clientId, sharedTopic: testOptions.shared_topic)
        context.appendClient(client: canaryClient)
    }
    ...
    mutating func run() async throws {
        ...
        while Date() < endTime {
            ...
            if testOptions.tpsSleepTime > SLEEP_OVERHEAD {
                try await Task.sleep(nanoseconds: testOptions.tpsSleepTime - SLEEP_OVERHEAD)
            }
            let targetTime = Date().addingTimeInterval(TimeInterval(testOptions.tpsSleepTime / NANO_SECOND))
