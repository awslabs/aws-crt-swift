...
class HTTPProxyTests: XCBaseTestCase {
  ...
  let HTTPProxyBasicAuthUsername = ProcessInfo.processInfo.environment[
    "AWS_TEST_BASIC_AUTH_USERNAME"]
  let HTTPProxyBasicAuthPassword = ProcessInfo.processInfo.environment[
    "AWS_TEST_BASIC_AUTH_PASSWORD"]
  ...
  func getConnectionType(type: ProxyTestType) -> HTTPProxyConnectionType {
    ...
    if type == ProxyTestType.tunnelingDoubleTLS || type == ProxyTestType.tunnelingHTTP
  ...
  func getProxyOptions(type: ProxyTestType, authType: HTTPProxyAuthenticationType) throws
    -> HTTPProxyOptions
  {
    HTTPProxyOptions(
  ...
  func getSSH(type: ProxyTestType) -> Bool {
    return
      !(type == ProxyTestType.forwarding || type == ProxyTestType.legacyHTTP
