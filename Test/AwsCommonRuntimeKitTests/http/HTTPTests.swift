...
class HTTPTests {
    ...
    func testGetHTTPSRequest() async throws {
        ...
        _ = try await HTTPClientTestFixture.sendHTTPRequest(method: "GET", endpoint: host, path: getPath, connectionManager: connectionManager)
        _ = try await HTTPClientTestFixture.sendHTTPRequest(method: "GET", endpoint: host, path: "/delete", expectedStatus: 404, connectionManager: connectionManager)
    }

    func testGetHTTPSRequestWithUtf8Header() async throws {
        let connectionManager = try await HTTPClientTestFixture.getHttpConnectionManager(endpoint: host, ssh: true, port: 443)
        let utf8Header = HTTPHeader(name: "TestHeader", value: "TestValueWithEmoji🤯")
        let headers = try await HTTPClientTestFixture.sendHTTPRequest(method: "GET", endpoint: host, path: "/response-headers?\(utf8Header.name)=\(utf8Header.value)", connectionManager: connectionManager).headers
        XCTAssertTrue(headers.contains(where: {$0.name == utf8Header.name && $0.value==utf8Header.value}))
    }

    func testGetHTTPRequest() async throws {
        let connectionManager = try await HTTPClientTestFixture.getHttpConnectionManager(endpoint: host, ssh: false, port: 80)
        _ = try await HTTPClientTestFixture.sendHTTPRequest(method: "GET", endpoint: host, path: getPath, connectionManager: connectionManager)
    }
    ...
    func testHTTPChunkTransferEncodingWithDataInLastChunk() async throws {
        ...
        let body: Response = try! JSONDecoder().decode(Response.self, from: httpResponse.body)
        XCTAssertEqual(body.data, HTTPClientTestFixture.TEST_DOC_LINE + String(decoding: lastChunkData, as: UTF8.self))
    }


    func testHTTPStreamIsReleasedIfNotActivated() async throws {
        do {
            let httpRequestOptions = try HTTPClientTestFixture.getHTTPRequestOptions(method: "GET", endpoint: host, path: getPath)
            let connectionManager = try await HTTPClientTestFixture.getHttpConnectionManager(endpoint: host, ssh: true, port: 443)
            let connection = try await connectionManager.acquireConnection()
