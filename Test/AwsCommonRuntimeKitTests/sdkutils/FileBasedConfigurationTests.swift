...
class FileBasedConfigurationTests: XCBaseTestCase {
  ...
  func testMergedCollectionFromPath() throws {
    ...
    let fileBasedConfiguration = try FileBasedConfiguration(
      configFilePath: profilePath, credentialsFilePath: configPath)
    ...
    let servicesSection = fileBasedConfiguration.getSection(
      name: "test-service", sectionType: .services)!
  ...
  func testResolveConfigPath() throws {
    ...
    XCTAssertEqual(
      try FileBasedConfiguration.resolveConfigPath(sourceType: .config), "\(home)/.aws/config")
    XCTAssertEqual(
      try FileBasedConfiguration.resolveConfigPath(sourceType: .credentials),
    ...
    XCTAssertEqual(
      try FileBasedConfiguration.resolveConfigPath(sourceType: .config), "/environment/.aws/config")
    XCTAssertEqual(
      try FileBasedConfiguration.resolveConfigPath(sourceType: .credentials),
    ...
    // from absolute path
    XCTAssertEqual(
      try FileBasedConfiguration.resolveConfigPath(
        sourceType: .config, overridePath: "/path/.aws/config"), "/path/.aws/config")
    XCTAssertEqual(
      try FileBasedConfiguration.resolveConfigPath(
        sourceType: .credentials, overridePath: "/path/.aws/credentials"), "/path/.aws/credentials")

    // from relative path
    XCTAssertEqual(
      try FileBasedConfiguration.resolveConfigPath(
        sourceType: .config, overridePath: "~/.aws/config"), "\(home)/.aws/config")
    XCTAssertEqual(
      try FileBasedConfiguration.resolveConfigPath(
        sourceType: .credentials, overridePath: "~/.aws/credentials"), "\(home)/.aws/credentials")
