...
class CredentialsProviderTests: XCBaseTestCase {
  ...
  func getClientBootstrap() throws -> ClientBootstrap {
    ...
    let hostResolver = try HostResolver(
      eventLoopGroup: elg,
      maxHosts: 8,
      maxTTL: 30)
  ...
  func testDelegateCredentialsProvider() async throws {
    ...
    do {
      ...
      do {
        let staticProvider = try CredentialsProvider(
          source: .static(
            accessKey: accessKey,
            secret: secret,
            sessionToken: sessionToken,
            shutdownCallback: getShutdownCallback()))
        delegateProvider = try CredentialsProvider(
  ...
  func testCredentialsProviderEnvThrow() async {
    let exceptionWasThrown = XCTestExpectation(
      description: "Exception was thrown because of missing credentials in environment")
    do {
      ...
      let provider = try CredentialsProvider(
        source: .profile(
          ...
          fileBasedConfiguration: FileBasedConfiguration(
            ...
            credentialsFilePath: Bundle.module.path(
              forResource: "example_credentials", ofType: "txt")!),
          shutdownCallback: getShutdownCallback()))
      let credentials = try await provider.getCredentials()
      XCTAssertNotNil(credentials)
      XCTAssertEqual("accessKey", credentials.getAccessKey())
    ...
    do {
      let staticProvider = try CredentialsProvider(
        source: .static(
          ...
          secret: secret,
          sessionToken: sessionToken))
      let cacheProvider = try CredentialsProvider(
        source: .cached(
