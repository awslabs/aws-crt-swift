...
class CredentialsTests: XCBaseTestCase {
  ...
  func testCreateAWSCredentials() async throws {
    ...
    let credentials = try Credentials(
      accessKey: accessKey, secret: secret, sessionToken: sessionToken, expiration: expiration)
    ...
    XCTAssertEqual(
      ...
      UInt64(credentials.getExpiration()!.timeIntervalSince1970))
  ...
  func testCreateAWSCredentialsInfinity() async throws {
    ...
    let credentials2 = try Credentials(
      accessKey: accessKey, secret: secret, sessionToken: sessionToken, expiration: expiration2)
  ...
  func testCreateAWSCredentialsWithoutSessionToken() async throws {
    ...
    let credentials = try Credentials(
      accessKey: accessKey, secret: secret, sessionToken: nil, expiration: expiration)
  ...
  func testCreateAWSCredentialsWithoutAccessKeyThrows() async {
    ...
    XCTAssertThrowsError(
      try Credentials(
        accessKey: accessKey, secret: secret, sessionToken: nil, expiration: expirationTimeout))
  }
}
